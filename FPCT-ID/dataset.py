...
def RelativeTransform(vs):
    if vs.shape[-1] == 0:
        return vs
    else:
        vs = rankdata(vs, method='min', axis=0)-1
        vs = (vs/vs.max()-0.5)*2
        return vs

class ModelNetDataLoader(Dataset):
    def __init__(self, root, epoch, aug = True, color_space = 'RGB', info_depth='vnc', num_points=12288, category='train', CRT=False, meta='Chinese_2016to2020_id+age+sex+label+face_FR.csv', nprseed=0):
        self.root = root
        self.aug = aug
        ...
        d = pd.read_csv(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'DataSplit', self.meta))
        self.labeldict = dict(zip(d.id,d.label))
    ...
    def __getitem__(self, index):
        ...
        if self.CRT:
            first_block = RelativeTransform(first_block)
        ...
        elif self.info_depth.endswith('s')+self.info_depth.endswith('b')+self.info_depth.endswith('v'):
        ...
        return torch.as_tensor(int(''.join(faceID.split('_')))), point_set.to(dtype=torch.float), torch.as_tensor(label,dtype=torch.long)
