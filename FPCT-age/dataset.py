...
def SecondaryTransform(vs,transform):
    if transform == 'exp':
        vs = np.exp(vs)
    elif transform == 'log':
        vs = np.log(2+vs)
...
class ModelNetDataLoader(Dataset):
    def __init__(self, root, epoch, aug = True, color_space = 'RGB', info_depth='vnc',num_points=12288, category='train', cross_group=1, meta='YourMeta.csv', nprseed=0):
        self.root = root
        self.aug = aug
